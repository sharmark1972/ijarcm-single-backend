...
export async function getVisitorAnalytics(timeframe: '1d' | '7d' | '30d' = '7d') {
  try {
    const apiUrl = typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3004') : 'http://localhost:3004';
    const response = await fetch(`${apiUrl}/api/visitors?timeframe=${timeframe}`, {
      headers: {
        'x-site-slug': 'ijarcm'
      }
    });
